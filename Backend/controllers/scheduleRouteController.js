const { ScheduleRoute, TrainSchedule, Station } = require('../models');

module.exports = {
  async getAllScheduleRoutes(req, res) {
    try {
      const routes = await ScheduleRoute.findAll({
        include: [TrainSchedule, Station]
      });
      res.json(routes);
    } catch (error) {
  ...
  async getScheduleRouteById(req, res) {
    try {
      ...
      if (!route) return res.status(404).json({ error: 'Schedule route not found' });
      res.json(route);
    } catch (error) {
  ...
  async createScheduleRoute(req, res) {
    try {
      ...
      const newRoute = await ScheduleRoute.create({ schedule_id, station_id, station_order, arrival_time, departure_time });
  ...
  async updateScheduleRoute(req, res) {
    try {
      ...
      const { schedule_id, station_id, station_order, arrival_time, departure_time } = req.body;
      await route.update({ schedule_id, station_id, station_order, arrival_time, departure_time });
      res.json(route);
    } catch (error) {
